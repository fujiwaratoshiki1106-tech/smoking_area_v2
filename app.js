// app.js — PWA版：CSV読込（自動探索）+ バッジ + 営業中判定 + 喫煙ラベル（紙OK/分煙/喫煙室）
//            + 検索クリアボタン + GoogleマップURLは「店名＋住所」を優先
  ...
  for (const p of candidates) {
    try {
      ...
      return rows.slice(1).map(r => {
        const obj = {};
        headers.forEach((k, i) => obj[k] = (r[i] ?? '').trim());
        return obj;
      });
    } catch (e) {
      lastErr = e;
      console.warn('CSV失敗:', p, e.message);
    }
  }
...
function isOpenNow(openHoursRaw) {
  ...
  const ranges = target.map(b => {
    ...
    const crosses = !!m[3] || e0 < s; // 翌 または 数値上の終了<開始
...
/* ========= GoogleマップURLを「店名＋住所」から生成 ========= */
function buildMapUrl(st) {
...
function render() {
  ...
  filtered.forEach((st, i) => {
    ...
    card.innerHTML = `
      ...
      <p>喫煙形態：${st.smoking || '-'}</p>
      <p>住所：${st.address || '-'}</p>
      <p><a href="${buildMapUrl(st)}" target="_blank" rel="noopener">Google Mapで見る</a></p>
    `;
